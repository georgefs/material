from .models import Live, Action, Player, Team, StatEvent, StatPlayer, StatPlayDetail
...
def extract_score_message(event):
    score_team = event['score_team'].location
    ...
    score_player = None
    score_player = [p for p in players if p.get('score', 0)][0]
    assert event['score_team'].pk == str(score_player['teamId'])
    try:
        score_player = StatPlayer.objects.get(id=score_player['playerId'])
    except Exception as e:
        score_player = StatPlayer.objects.create(
                            id=score_player['playerId'],
                            first_name=score_player['firstName'],
                            last_name=score_player['lastName'],
                            team=event['score_team'],
                            info=json.dumps(score_player)
                        )
    logger.info('----actions-----')
    action = event['playEvent']
    action = StatPlayDetail.objects.get(event_id=action['playEventId'], detail_id=action['playDetail']['playDetailId'])
    logger.info('----extract score message---\n\n')
...
def live_events(live_id):
    for event in StatEvent.live_events(live_id):
        if event['type'] == 'score':
...
def tagger(video_id, video_end=False):
    ...
    for idx, point_info in enumerate(point_infos):
        ...
        if score_action == '罚球':
            ...
            videoscene.start = videoscene.end - 5
        videoscene.save()
